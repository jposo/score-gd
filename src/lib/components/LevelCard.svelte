...
<div class="indicator w-full">
    {#if props.spotlightType}
        <span
            class={`indicator-item badge badge-sm ${spotlightBadgeClass} lowercase font-semibold z-3`}
        >
            {props.spotlightType}
        </span>
    {/if}
        ...
        >
            ...
            <div class="text-sm opacity-90">
                by <span class="font-medium"
                    >{props.publisher ?? "unknown"}</span
                >
            </div>

            <div class="flex flex-wrap items-center gap-2 text-xs">
                <div
                    class="badge border-white/40 bg-black/30 text-neutral-content"
                >
                    <span class="font-semibold"
                        >{props.difficulty.toLowerCase()}</span
                    >
                </div>
                <div
                    class="badge border-white/40 bg-black/30 text-neutral-content"
