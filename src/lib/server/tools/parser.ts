...
export class Parser {
  ...
  private static resolveLength(length: string) {
    if (length === "0") {
      return Length.Tiny;
    } else if (length === "1") {
      return Length.Short;
  ...
  static parseLevelObject(raw: string) {
    ...
    const length = Parser.resolveLength(result["15"]);
    ...
    return {
      ...
      length,
      songId:
        result["12"] == "0" ? parseInt(result["35"]) : -parseInt(result["12"]),
      gameVersion: parseInt(result["13"]),
      coins: parseInt(result["37"]),
      twoPlayer: result["31"] === "1",
    };
  }
  ...
  static parseLevelSearch(raw: string): LevelSearchResult[] {
    ...
    return levelSearch.levels.map((level) => {
      ...
      let song =
        levelSearch.songs.find((song) => song.id === level.songId) ?? null;
